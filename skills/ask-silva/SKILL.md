---
name: ask-silva
description: Ask which skill or flow fits your situation. A router over the engineering skills, with the ADHD-friendly output contract applied throughout.
disable-model-invocation: true
---

<!-- Fork of mattpocock/skills skills/engineering/ask-matt/SKILL.md @ 6654f6b60cd9 (2026-08-24). Diff against upstream before re-syncing. -->
...
Every chat reply and every artifact these skills produce (spec, ticket, brief, handoff, report) is written in the `/unglaze` voice: conclusion first, one idea per bullet, no repetition, nothing that doesn't carry information. Budgets on top of that voice:

- **Chat reply**: 150 words, three findings or options. More exist? Say how many and stop.
- **Artifact over 300 words**: opens with an `At a glance` block of at most 100 words: problem, decision or status, next action.
- **Lists**: the smallest complete set of non-overlapping items. Never "extensive".

Brevity compresses structure and repetition, never substance: risks, acceptance criteria, negations, and unresolved decisions survive verbatim. Two skills below (`/to-spec`, `/handoff`) are silvabyte forks that bake this contract into their templates; the rest are upstream and rely on this section.
...
- **`/unglaze`** is the *voice* vocabulary: the output contract above is its budgets; the skill itself is the full rulebook (banned phrases, sacred details, anti-patterns). Invoke it directly on any output that still glazes.
